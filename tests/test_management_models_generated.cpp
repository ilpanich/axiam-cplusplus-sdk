...
AXIAM_TEST("management model AssignRoleToGroupRequest round-trips without losing a field") {
    const auto wire = nlohmann::json::parse(
        R"json({"group_id": "11111111-1111-4111-8111-111111111111", "inherit": true, "resource_id": "11111111-1111-4111-8111-111111111111", "tenant_scope": ["11111111-1111-4111-8111-111111111111"]})json");
...
AXIAM_TEST("management model AssignRoleToServiceAccountRequest round-trips without losing a field") {
    const auto wire = nlohmann::json::parse(
        R"json({"inherit": true, "resource_id": "11111111-1111-4111-8111-111111111111", "service_account_id": "11111111-1111-4111-8111-111111111111", "tenant_scope": ["11111111-1111-4111-8111-111111111111"]})json");
...
AXIAM_TEST("management model AssignRoleToUserRequest round-trips without losing a field") {
    const auto wire = nlohmann::json::parse(
        R"json({"inherit": true, "resource_id": "11111111-1111-4111-8111-111111111111", "tenant_scope": ["11111111-1111-4111-8111-111111111111"], "user_id": "11111111-1111-4111-8111-111111111111"})json");
...
AXIAM_TEST("management model CertificatePolicy round-trips without losing a field") {
    const auto wire = nlohmann::json::parse(
        R"json({"default_cert_validity_days": 1, "max_cert_validity_days": 1, "server_cert_allowed_names": ["example"]})json");
...
AXIAM_TEST("management model CimdPolicy round-trips without losing a field") {
    const auto wire = nlohmann::json::parse(
        R"json({"allow_http": true, "confidential_only": true, "enabled": true, "max_cache_secs": 1, "max_metadata_bytes": 1, "min_cache_secs": 1, "restrict_same_domain": true, "trusted_client_id_domains": ["example"], "trusted_redirect_domains": ["example"]})json");

    const auto value = wire.get<CimdPolicy>();
    const nlohmann::json encoded = value;
    // Exact equality, not "the fields I remembered to check". The wire object above carries
    // every property the spec declares, so a dropped field and an invented one both fail here.
    AXIAM_CHECK(encoded == wire);
    ...
    // And encoding is a fixed point -- a second pass changes nothing.
    const nlohmann::json again = encoded.get<CimdPolicy>();
    AXIAM_CHECK(again == encoded);
}
...
AXIAM_TEST("management model CreateRegistrationTokenRequest round-trips without losing a field") {
    const auto wire = nlohmann::json::parse(
        R"json({"expires_in_hours": 1, "name": "example"})json");

    const auto value = wire.get<CreateRegistrationTokenRequest>();
    const nlohmann::json encoded = value;
    // Exact equality, not "the fields I remembered to check". The wire object above carries
    // every property the spec declares, so a dropped field and an invented one both fail here.
    AXIAM_CHECK(encoded == wire);
    ...
    // And encoding is a fixed point -- a second pass changes nothing.
    const nlohmann::json again = encoded.get<CreateRegistrationTokenRequest>();
    AXIAM_CHECK(again == encoded);
}

AXIAM_TEST("management model RegistrationTokenResponse round-trips without losing a field") {
    const auto wire = nlohmann::json::parse(
        R"json({"created_at": "2026-08-26T00:00:00Z", "created_by": "11111111-1111-4111-8111-111111111111", "expires_at": "2026-08-26T00:00:00Z", "id": "11111111-1111-4111-8111-111111111111", "name": "example", "tenant_id": "11111111-1111-4111-8111-111111111111", "used_at": "2026-08-26T00:00:00Z", "used_by_client_id": "example"})json");

    const auto value = wire.get<RegistrationTokenResponse>();
    const nlohmann::json encoded = value;
    // Exact equality, not "the fields I remembered to check". The wire object above carries
    // every property the spec declares, so a dropped field and an invented one both fail here.
    AXIAM_CHECK(encoded == wire);
    ...
    // And encoding is a fixed point -- a second pass changes nothing.
    const nlohmann::json again = encoded.get<RegistrationTokenResponse>();
    AXIAM_CHECK(again == encoded);
}

AXIAM_TEST("management model CreateRegistrationTokenResponse round-trips without losing a field") {
    const auto wire = nlohmann::json::parse(
        R"json({"initial_access_token": "example", "token": {"created_at": "2026-08-26T00:00:00Z", "created_by": "11111111-1111-4111-8111-111111111111", "expires_at": "2026-08-26T00:00:00Z", "id": "11111111-1111-4111-8111-111111111111", "name": "example", "tenant_id": "11111111-1111-4111-8111-111111111111", "used_at": "2026-08-26T00:00:00Z", "used_by_client_id": "example"}})json");

    const auto value = wire.get<CreateRegistrationTokenResponse>();
    const nlohmann::json encoded = value;
    // Exact equality, not "the fields I remembered to check". The wire object above carries
    // every property the spec declares, so a dropped field and an invented one both fail here.
    AXIAM_CHECK(encoded == wire);
    ...
    // And encoding is a fixed point -- a second pass changes nothing.
    const nlohmann::json again = encoded.get<CreateRegistrationTokenResponse>();
    AXIAM_CHECK(again == encoded);
}
...
AXIAM_TEST("management model RoleUserAssignment round-trips without losing a field") {
    const auto wire = nlohmann::json::parse(
        R"json({"inherit": true, "resource_id": "11111111-1111-4111-8111-111111111111", "tenant_scope": ["11111111-1111-4111-8111-111111111111"], "user": {"created_at": "2026-08-26T00:00:00Z", "email": "example", "email_verified": true, "failed_login_attempts": 1, "id": "11111111-1111-4111-8111-111111111111", "is_locked": true, "locked_until": "2026-08-26T00:00:00Z", "metadata": {}, "mfa_enabled": true, "status": "Active", "tenant_id": "11111111-1111-4111-8111-111111111111", "updated_at": "2026-08-26T00:00:00Z", "username": "example"}})json");
...
AXIAM_TEST("management model WebauthnPolicy round-trips without losing a field") {
    const auto wire = nlohmann::json::parse(
        R"json({"webauthn_user_verification": "example"})json");

    const auto value = wire.get<WebauthnPolicy>();
    const nlohmann::json encoded = value;
    // Exact equality, not "the fields I remembered to check". The wire object above carries
    // every property the spec declares, so a dropped field and an invented one both fail here.
    AXIAM_CHECK(encoded == wire);
    ...
    // And encoding is a fixed point -- a second pass changes nothing.
    const nlohmann::json again = encoded.get<WebauthnPolicy>();
    AXIAM_CHECK(again == encoded);
}
...
AXIAM_TEST("management enum CertificateType maps every value both ways") {
    ...
    AXIAM_CHECK(to_wire(CertificateType::Server) == "Server");
    AXIAM_CHECK(certificate_type_from_wire("Server") == CertificateType::Server);
...
AXIAM_TEST("management enum ClientAuthMethod maps every value both ways") {
    ...
    AXIAM_CHECK(to_wire(ClientAuthMethod::None) == "none");
    AXIAM_CHECK(client_auth_method_from_wire("none") == ClientAuthMethod::None);
...
AXIAM_TEST("management enum ManagedBy maps every value both ways") {
    AXIAM_CHECK(to_wire(ManagedBy::Admin) == "admin");
    AXIAM_CHECK(managed_by_from_wire("admin") == ManagedBy::Admin);
    AXIAM_CHECK(to_wire(ManagedBy::Dcr) == "dcr");
    AXIAM_CHECK(managed_by_from_wire("dcr") == ManagedBy::Dcr);
    AXIAM_CHECK(to_wire(ManagedBy::Cimd) == "cimd");
    AXIAM_CHECK(managed_by_from_wire("cimd") == ManagedBy::Cimd);

    // An unrecognised value DECODES, to an enumerator of its own -- it is not reported, and it
    // is not mapped to whichever known enumerator happens to be first (§27.11 rule 1). Throwing
    // here would fail the whole response the value arrived in, so one field of one record would
    // take down the page it was on, including the records the caller did ask for.
    AXIAM_CHECK(managed_by_from_wire("__not_a_managed_by__") == ManagedBy::Unknown);
    ...
    // The JSON hooks must agree with the wire functions, or a model carrying this enum encodes
    // differently from the enum itself.
    const nlohmann::json j = ManagedBy::Admin;
    AXIAM_CHECK(j.get<std::string>() == "admin");
    AXIAM_CHECK(j.get<ManagedBy>() == ManagedBy::Admin);
}
...
AXIAM_TEST("management federation: re-scoping returns a new handle (§27.4 rule 3)") {
    auto fixture = axtest::mgmt::signed_in_two(200, R"json({"items": [{"allow_tenant_inheritance": true, "allowed_algorithms": ["example"], "allowed_issuer_tenants": ["example"], "apple_key_id": "example", "apple_team_id": "example", "attribute_map": {}, "authorization_endpoint": "example", "button_icon": "example", "client_id": "example", "created_at": "2026-08-26T00:00:00Z", "effective_scopes": ["example"], "enabled": true, "has_bundled_mark": true, "id": "11111111-1111-4111-8111-111111111111", "metadata_url": "example", "mints_client_secret": true, "pkce_required": true, "protocol": "example", "provider": "example", "provider_kind": "example", "provider_slug": "example", "scopes": ["example"], "tenant_id": "11111111-1111-4111-8111-111111111111", "token_endpoint": "example", "token_exchange": {"accepted_audiences": ["example"], "enabled": true, "max_lifetime_secs": 1, "max_token_age_secs": 1, "scope_map": {}, "subject_mapping": "example"}, "updated_at": "2026-08-26T00:00:00Z", "userinfo_endpoint": "example"}], "total": 1, "offset": 0, "limit": 50})json",
                                              200, R"json({"items": [{"allow_tenant_inheritance": true, "allowed_algorithms": ["example"], "allowed_issuer_tenants": ["example"], "apple_key_id": "example", "apple_team_id": "example", "attribute_map": {}, "authorization_endpoint": "example", "button_icon": "example", "client_id": "example", "created_at": "2026-08-26T00:00:00Z", "effective_scopes": ["example"], "enabled": true, "has_bundled_mark": true, "id": "11111111-1111-4111-8111-111111111111", "metadata_url": "example", "mints_client_secret": true, "pkce_required": true, "protocol": "example", "provider": "example", "provider_kind": "example", "provider_slug": "example", "scopes": ["example"], "tenant_id": "11111111-1111-4111-8111-111111111111", "token_endpoint": "example", "token_exchange": {"accepted_audiences": ["example"], "enabled": true, "max_lifetime_secs": 1, "max_token_age_secs": 1, "scope_map": {}, "subject_mapping": "example"}, "updated_at": "2026-08-26T00:00:00Z", "userinfo_endpoint": "example"}], "total": 1, "offset": 0, "limit": 50})json");
    auto mgmt = fixture.client.management();
