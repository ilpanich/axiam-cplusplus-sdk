...
- **Re-vendored the AXIAM contract artifacts at contract 1.36.** `CONTRACT.md`,
  ...
  (ilpanich/axiam#396). `opaque-test-vectors.json` did not change in 1.36 and is
  untouched; this SDK vendors no `proto/` (it is REST-only, and 1.36 changed no
  proto anyway). No SDK code changes with them; the three entries below are why
  not.
