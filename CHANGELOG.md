...
- **`CreateRegistrationTokenResponse::initial_access_token` is now
  `Sensitive<std::string>`, was `std::string`** (CONTRACT.md §27.5, contract
  1.50, #480). The RFC 7591 §1.2 initial access token is returned exactly once
  and never retrievable, but it was missing from the registry's curated
  `(schema, field)` table, so the generator emitted a bare `std::string` and
  the credential appeared in every stream and log rendering of the model — the
  leak §7 rule 1 and §27.5 exist to prevent.
  ...
  Migration — read the token through the explicit reveal, at the one point of
  use:

  ```cpp
  auto created = client.management().oauth2_clients().create_registration_token(body);
  // before: const std::string token = created.initial_access_token;
  const std::string& token = axiam::detail::reveal(created.initial_access_token);
  ```

  There is deliberately **no** plain-`std::string` accessor kept alongside it:
  the plain accessor is precisely the leak (contract 1.50). `to_json` /
  `from_json` go through `detail::reveal` and the `Sensitive` constructor, so
  the JSON on the wire is unchanged — `openapi.json` did not move, and there is
  no `proto/` here to move (§1.1).
...
- **`CONTRACT.md` (1.50) and `management-registry.json` re-synced from a merged
  ...
  `python3 scripts/gen_management.py`, nothing hand-edited — still **162
  operations across 24 namespaces**, the one member above being the only
  generated change (plus the §27.5 rule 3 once-only note on
  `create_registration_token`). The README's conformance statement names no
  contract version, so it is unchanged.
...
- Sign-CSR certificates and passkey first-login setup (F-1) (#61)

- **CONTRACT.md §27 — `certificates().sign_csr()` (contract 1.45).**
  `POST /api/v1/certificates/sign-csr`, generated by `scripts/gen_management.py`
  from the re-vendored registry like every other operation on this surface: the
  management operation count moves 159 → 160 across the same 24 namespaces, and
  the surface test asserting that number (`tests/test_management_generated.cpp`)
  moved with it.

  The response is the **existing** `Certificate` model, deliberately not
  `GeneratedCertificate` — whose `private_key_pem` field is mandatory and would
  always be absent here, since the key stays with whoever generated the CSR and
  AXIAM never sees it. A model round-trip test in
  `tests/test_management_semantics.cpp` asserts `Certificate` carries no
  private-key field in either direction, and that a server response carrying a
  stray one is not silently absorbed. `SignCertificateCsrRequest` is generated,
  not hand-written: the vendored registry already carries its request schema.

- **CONTRACT.md §24.1 / §25.2 rule 2 — a passkey or security key as the first
  factor (contract 1.45).** `webauthn_setup_register_start()` /
  `webauthn_setup_register_finish()`, the WebAuthn twin of `mfa_setup_enroll()`
  / `mfa_setup_confirm()`: reached exactly where the TOTP pair is (when
  `LoginResult::mfa_setup_required` is set), taking the same setup token as
  their only credential, and `finish` adopting a session exactly as
  `mfa_setup_confirm()` does — including clearing the §17 decision memo,
  because it **is** the completion of the login the tenant's MFA policy
  interrupted.

  **Neither call carries this client's session credential, even when one is
  configured** — the setup token is the only credential §24.1 permits on
  either request. For this SDK's cookie-jar transport (§4) that is not free:
  `CurlTransport::perform_isolated()` routes both calls off the pool's shared
  handles onto a fresh, unshared handle with no cookie engine at all, so a
  session cookie already sitting in the jar is never replayed — and, because
  `finish` still needs to *adopt* the session its own response sets, any
  `Set-Cookie` it receives is merged back into the shared jar explicitly
  (`CurlTransport::merge_into_shared_jar()`) rather than relying on an engine
  that was deliberately not there for the outgoing half. Both properties are
  asserted against the real libcurl transport in
  `tests/test_integration_curl.cpp`, not only against the in-memory fake, since
  the fake has no cookie jar to leak a session out of in the first place.

  `setup_token` follows `state_token` into `Sensitive<std::string>` (§7, §24.5,
  §25.3).
...
- **CONTRACT.md §10.4 — an optional session-revocation feed poller (contract
  ...
  `AuthenticatorOptions::revocation_feed`.
  ...
  Nothing changes unless you set it: the field is null by default and the
  authenticator is exactly the one it was. It never fetches on the request path
  after the first call, and a feed that is down is not retried on every request
  (the interval runs from the last *attempt*). And it never fails closed: an
  unreachable feed, a non-`200`, an unparseable body or an unknown `alg` all
  behave exactly as no feed at all, and specifically not as an empty list, which
  would assert that nothing has been revoked. A token with no `sid` is never
  matched against it, and there is no fallback to `jti`. `RevocationFeed` is
  thread-safe, so one instance can serve several authenticators.
...
- Two tests pinning CONTRACT.md §16 against the server's new answer for a
  ...
  changed: §16.3 already retried `5xx` on an eligible operation and the retry
  policy already honoured `Retry-After` as a floor. Both halves are asserted
  through the public surface with a wire count, because a retry policy nobody
...
- **A malformed `mtls_endpoint_aliases` entry now throws instead of falling
  ...
  reported it as a transient one. Both `preferred_endpoint` overloads apply it,
  so no call site changed; it is per endpoint, and a client built without
  `with_client_cert` never reads the member at all.
...
- **The four public login-provider operations (CONTRACT §12.1, contract 1.38).**
  `Client::sso_providers()`, `Client::sso_start_oauth2()`,
  `Client::sso_complete_oauth2()` and `Client::sso_complete_handoff()` bring §12
  to thirteen operations, under the snake_case names §12.2 reserves for C++.
  They are what a login *page* needs: which "Sign in with X" buttons to render,
  and how to finish the two flows that cannot set a `SameSite=Strict` cookie on
  their own response. `FederationProvider` models the public provider shape
  faithfully, including the nullable `button_icon` data URL, `has_bundled_mark`
  and `inherited`; `kHandoffQueryParam` and `kHandoffCodeTtlSeconds` carry the
  handoff mechanism's two constants.

  Five rules are load-bearing, and each has a test.

  - **An empty vector is a success, and the only success there is** (note 9). An
    unknown organization, a known one with no providers, and a request naming no
    organization at all all answer `200` with an empty array. `sso_providers()`
    never turns that into a not-found error and never refuses client-side for
    missing workspace context — a client-side `400` would restore exactly the
    two-valued organization-slug oracle the empty list exists to remove.
  ...
  - **A `400` is a configuration error** (rule 12a, new at 1.38). §2 puts it on
    the `NetworkError` row, as distinct from the `AuthError` a `401` gets, and
    it is not retried.
...
- **Re-vendored contract 1.38.** `CONTRACT.md`, `openapi.json` and
  ...
  vendored artifacts move: `openapi.json` gained fields on the
...
- **§5.2.2 rule 4 is new, and is an errata rather than a wire change.** The
  ...
  every request sends `impl.tenant_header` under `X-Tenant-ID`; no endpoint is
  special-cased.
...
- **§27: service accounts as RBAC principals** (contract 1.34) — eight generated
  operations across `roles()`, `groups()` and `service_accounts()`, with the
  `AssignRoleToServiceAccountRequest`, `RoleServiceAccountAssignment` and
  `AddServiceAccountMemberRequest` models they need.
  `roles().unassign_from_service_account()` takes the same optional `resource_id`
...
- **§5.2.2: the acting tenant and the principal tenant are different things**
  (contract 1.34). `UserInfo` gains `principal_tenant_id`,
  `principal_tenant_slug`, `org_id` and `reachable_tenant_ids` — appended after
  `organization_level` and defaulted, for the same reason that one was: every
  existing aggregate initializer still compiles.
...
- **§5.2.3: tenant-scoped role assignments** (contract 1.35). `tenant_scope`
  appears on the three assignment request structs and on the assignment objects
  the read paths return. Disengaged means unrestricted, which is what every
  assignment written before the field existed already meant.

  `reachable_tenant_ids` pairs with it on `UserInfo`: a narrowed
  organization-level principal still reports `organization_level == true`, so an
...
- **A registration record for your own password was sealed against the wrong
  ...
  `opaque_enrollment_for_self()` seals against the principal tenant captured at
  login (naming it by id and sending no `tenant_slug`, which would otherwise
  out-vote the id server-side) and is what a self-service password change must
  call. It throws a `NetworkError` before any login has completed, because there
  is nothing to seal against then and falling back to the acting tenant is the
  bug itself.
...
- **`tenant_scope: []` no longer reaches the wire** (§5.2.3 rule 1, refused with
  `400`). The generated guard for an optional member is `if (value.field)`, which
  an *engaged* `std::optional` holding an empty vector passes — and an empty vector
  is exactly what building the field from a filtered collection produces for "no
  tenants named". `scripts/gen_management.py` now emits
  `if (value.tenant_scope && !value.tenant_scope->empty())` for that one field.
